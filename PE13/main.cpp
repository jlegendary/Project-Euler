...
int main()
{
    ...
    for(j = COLUMNS - 1; j >= 0; --j)
    {
        ...
        for(i = 0; i < LINES; ++i)
            i_result += numbers[i][j];
        
        c_result[j] = i_result % 10 + '0';
        i_carry += i_result / 10;
    }
    
    sprintf(c_carry, "%i", i_carry);
    strcpy(c_total, c_carry);
    strncat(c_total, c_result, 10 - strlen(c_carry));
